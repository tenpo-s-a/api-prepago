...
public class Test_PrepaidAccountingEJBBean10_createCSV extends TestBaseUnit {
  ...
  public void createCSV() throws Exception {
    ...
    ArrayList<AccountingData10> movements = new ArrayList<>();

    // Crear los movimientos a insertar
    AccountingData10 mov1 = createAccountingMovement(1L, AccountingTxType.CARGA_WEB, 10000L, 15L, 100L, 200L, 38L);
    movements.add(mov1);

    AccountingData10 mov2 = createAccountingMovement(2L, AccountingTxType.CARGA_POS, 20000L, 30L, 200L, 400L, 76L);
    movements.add(mov2);

    AccountingData10 mov3 = createAccountingMovement(3L, AccountingTxType.RETIRO_WEB, 8000L, 11L, 70L, 40L, 8L);
    movements.add(mov3);

    getPrepaidAccountingEJBBean10().createAccountingCSV(fileName, movements);

    // Cargar los movimientos existentes en el archivo
    List<AccountingData10> movementsOut = getCsvData(fileName);
    ...
    for (int i = 0; i < movements.size(); i++) {
      AccountingData10 originalMov = movements.get(i);
      AccountingData10 outputMov = movementsOut.get(i);
  ...
  AccountingData10 createAccountingMovement(Long id, AccountingTxType type, Long ammount, Long ammountUsd, Long exchangeDif, Long fee, Long iva) {
    AccountingData10 mov = new AccountingData10();
  ...
  public List<AccountingData10> getCsvData(String fileName) throws Exception {
    ...
    List<AccountingData10> lstAccounting10;
    try {
      ...
      while ((record = csvReader.readNext()) != null) {
        AccountingData10 accountingMov = createAccountingMovement(Long.valueOf(record[0]),

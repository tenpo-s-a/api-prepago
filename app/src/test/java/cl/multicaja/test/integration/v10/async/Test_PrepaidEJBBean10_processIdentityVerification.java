...
public class Test_PrepaidEJBBean10_processIdentityVerification extends TestBaseUnitAsync {
  ...
  /**
   * Segun historia (https://www.pivotaltracker.com/story/show/162347418)
   */

  /**
   * Caso 1:
   *  Si marca todos si, se deberán actualizar los estados  en api users
   *    - nombre : a validado
   *    - rut: a validado
  ...
  public void allYes() throws Exception {
    ...
    try{
      ...
      Assert.assertEquals("Debe tener rut.status VERIFIED", RutStatus.VERIFIED, verifiedUser.getRut().getStatus());
      Assert.assertEquals("Debe tener nameStatus VERIFIED", NameStatus.VERIFIED, verifiedUser.getNameStatus());
      Assert.assertEquals("Debe tener identityStatus VERIFIED", UserIdentityStatus.NORMAL, verifiedUser.getIdentityStatus());
  ...
  /**
   * Caso 2:
   *  Si marca todos sí menos el de validación de listas negras se deberán actualizar los estados  en api users
   *    - identity status: terrorista
  ...
  public void retryValidation() throws Exception {
    ...
    IdentityValidation10 identityValidation10 = new IdentityValidation10();
    identityValidation10.setIsCiValid("No");
    identityValidation10.setIsGsintelOk("Si");
    identityValidation10.setUserPhotoMatchesCi("Si");
    identityValidation10.setNameAndLastnameMatchesCi("Si");
    identityValidation10.setRutMatchesCi("Si");
    identityValidation10.setNewLastname("");
    identityValidation10.setNewName("");

...
public class Test_TecnocomReconciliationEJBBean10_insertAutorization extends TestBaseUnitAsync {
  ...
  // Anulacion Internacional (DB = no, tipo = OP)
  @Test
  public void processTecnocomTableData_whenMovNotInDB_IsRevesedPurcharseOp_movIsInsertedAndLiqAccMustExistInInitialState() throws Exception {
    // Inserta movimiento que vino en archivo OP
    MovimientoTecnocom10 movimientoTecnocom10 = createMovimientoTecnocom(tecnocomReconciliationFile10.getId());
    ...
    List<PrepaidMovementFee10> prepaidMovementFee10List = getPrepaidMovementEJBBean11().getPrepaidMovementFeesByMovementId(prepaidMovement10.getId());
    Assert.assertEquals("Debe tener 1 fees asignadas", 1, prepaidMovementFee10List.size());
    ...
    // Anulaciones internacionales: Regla de contabilidad 1: El monto_trx_pesos se llenara con el monto del archivo de operacion impfac
    Assert.assertEquals("impfac de tecnocom debe ser igual al de contabilidad", movimientoTecnocom10.getImpFac(), acc.getAmount());

    // Anulaciones internacionales: Regla de contabilidad 2: Dif tipo cambio debe rellenarse con la suma de las comisiones
